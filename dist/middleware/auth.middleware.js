...
const verifyUserToken = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (!token) {
            return next(new apiError_util_1.default('token not found', statusCode_util_1.default.UNAUTHORIZED));
        }
        const user = yield (0, user_service_1.GetUserByAuthtoken)(token);
        if (!user) {
            return next(new apiError_util_1.default('Invalid token', statusCode_util_1.default.UNAUTHORIZED));
        }
        req.user = user;
        next();
    }
        ...
        console.log(error);
        return next(new apiError_util_1.default(error, statusCode_util_1.default.INTERNAL_SERVER_ERROR));
    }
});
